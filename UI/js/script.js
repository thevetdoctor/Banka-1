document.addEventListener('DOMContentLoaded', () => {
  ...
  const toggleClickedItem = ({ target }) => {
    ...
    if (target.classList.contains('fa-dashboard') || target.classList.contains('fa-book')) {
      secondNav.classList.toggle('show');
    } else if (secondNav && !secondNav.classList.contains('show')) secondNav.classList.add('show');
